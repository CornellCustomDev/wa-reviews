...
$classes = Flux::classes()
    ...
    ->add('shadow-xs')
    ->add('bg-linear-to-b from-[#fcfcfc] to-[#eeeeee]')
    ->add('dark:bg-white/10 dark:disabled:bg-white/[7%]')
    // Make the placeholder match the text color of standard input placeholders...
    ->add('disabled:shadow-none')
    ->add(match ($size) {
        default => 'h-10 text-base font-systemUi rounded-[5px] px-3 block w-full',
        'sm' => 'h-8 text-sm rounded-md pl-3 pr-2 block w-full',
        'xs' => 'h-6 text-xs rounded-md pl-3 pr-2 block w-full',
    })
    ->add($invalid
        ? 'border border-red-500'
        : 'border border-cds-gray-400 border-b-cds-gray-400 dark:border-white/10'
    )
    ;
@endphp
...
<button type="button" {{ $attributes->class($classes) }} @if ($invalid) data-invalid @endif data-flux-group-target data-flux-select-button>
    ...
    <flux:icon.chevron-up-down class="text-white in-[[disabled]]:text-cds-gray-50! dark:text-white/60 bg-cds-blue-600/95 w-7 h-10 px-1 -mt-2 -mb-2 -mr-3 dark:[[data-flux-select-button]:hover_&]:text-white dark:in-[[disabled]]:text-white/40!" />
</button>
